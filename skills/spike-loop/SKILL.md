...
ヘッドレス Claude で `/vibecorp:ship-parallel` を実行し、command-log を監視して stuck 検出 → 診断スナップショット → kill + cleanup → 分析レポートを自律的にループする。
...
/vibecorp:spike-loop --max-runs 5    # 最大5回ループ（デフォルト: 3）
```
...
```bash
claude -p --permission-mode dontAsk --verbose "/vibecorp:ship-parallel <Issue URL 1> <Issue URL 2> [...]"
```
...
**修正の自動適用は行わない**（Phase 2 で対応予定）。分析レポートを出力し、ユーザーの判断を待つ。ユーザーが修正を指示した場合は、修正後に再度 `/vibecorp:spike-loop` を実行する。
