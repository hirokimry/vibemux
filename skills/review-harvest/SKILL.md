...
**重要（課金）**: `ANTHROPIC_API_KEY` 設定時は従量課金。未設定なら Claude Max サブスク使用。`/vibecorp:autopilot` 連続実行時はコストが発生する。コスト制御は `VIBECORP_HARVEST_MAX_PRS` と C*O 5 呼出上限、30K トークン切り詰めのみ。

## 使用方法

```bash
/vibecorp:review-harvest                    # 前回収集以降の PR を走査
/vibecorp:review-harvest --worktree <path>  # worktree 内で実行
```
...
## /vibecorp:session-harvest との責務境界

- **/vibecorp:review-harvest**: マージ済み PR のレビュー指摘を入力として knowledge を更新
- **/vibecorp:session-harvest**: セッション中の会話を入力として knowledge を更新
